...
One page. Everything else is in
[docs/STUDY-GUIDE.md](docs/STUDY-GUIDE.md) (how to study, config, troubleshooting)
and [docs/CONCEPTS.md](docs/CONCEPTS.md) (every concept used).
...
> so answers come from the syllabus and cite the page.
...
**Debugging rule:** decide which pipeline first. Right chunk found but bad
answer = generation problem (prompt). Right chunk never found = retrieval
...
| **4** | Chunks retrieved per question | Enough context without dilution |
| **0.7** | Max cosine distance | Above this = irrelevant, discarded |
| **31** | Tests passing | |
...
| `RETRIEVAL_TOP_K` | More/fewer chunks | no |
...
book you ask for. Google never tells you a model when issuing a key.
...
> The question is embedded by the same model that embedded every chunk of the
> PDF. pgvector compares them by cosine distance and returns the four closest.
> Only those go to the language model. *(Show the citations with distances.)*
...
> not exist. This answers only from the uploaded PDF, cites the page, and
> refuses when the topic is absent. *(Ask it something off-syllabus, live.)*
...
> chunk-level, so on short documents the cited page can be adjacent to the exact
> sentence. Single machine only. Nepali content retrieves less reliably — the
> embedding model is mostly English-trained.
...
| **0.7 distance threshold** | Without it, an off-topic question still returns its four closest chunks and gets answered from junk |

---

## Three defects fixed from the original brief
...
- [ ] `manage.py test services.tests` → 31 pass
...
report the percentage correct. **A measured accuracy number is worth more than
any new feature** — and it turns "it works" into evidence.
